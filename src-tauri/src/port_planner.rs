...
pub fn plan_launch(project: &ManagedProject, registry: &PortRegistry, reserve_ports: bool) -> SmartLaunchPlan {
    ...
    for command in [project.backend.as_ref(), project.frontend.as_ref()].into_iter().flatten() {
        ...
        if let Some(port) = preferred {
            ...
            if reserve_ports && !local_blocked {
                reserve(command_key(project, command), picked, registry);
                reservations.push(picked);
            }
        }

        if local_blocked {
            blocked = true;
            warnings.extend(warning_list.clone());
        }

        services.push(SmartLaunchServicePlan {
...
fn command_key(project: &ManagedProject, command: &CommandConfig) -> String { format!("{}:{}", project.id, command.id) }
...
fn default_port(command: &CommandConfig) -> Option<u16> {
    ...
    if command.kind == "yarn-frontend" || ["yarn", "npm", "pnpm"].contains(&command.executable.to_lowercase().as_str()) { return Some(5173); }
    None
}
...
fn adapt_command_port(command: &CommandConfig, assigned_port: u16) -> Option<CommandConfig> {
    ...
    if exe == "yarn" || exe == "npm" || exe == "pnpm" || adapted.kind == "yarn-frontend" {
        let mut next_args = Vec::new();
...
mod tests {
    use super::*;
    use crate::models::{ProjectDetection, ProjectUrls};
    ...
    fn reservation_created_when_adaptable() {
        let project = ManagedProject {
            ...
            backend: Some(CommandConfig { id: "backend".into(), label: "b".into(), executable: "symfony".into(), args: vec!["server:start".into()], working_directory: ".".into(), kind: "symfony-backend".into(), stop_command: None, risky: None, preferred_port: Some(8000), port_strategy: None, smart_port_pattern: None, env: None, origin: None, overridden_by_user: None }),
            frontend: None, urls: ProjectUrls { backend: None, frontend: None }, detection: ProjectDetection::default(), runtime: None, smart_ports_enabled: Some(true), stack_plan: None, ai_metadata: None,
        };
